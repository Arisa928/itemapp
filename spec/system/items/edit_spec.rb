...
RSpec.describe "items", type: :system do
  ...
  let!(:item1) { create(:item, user: user, rakuten_url: 'http://image_url1') }
  ...
  describe "rakuten_url" do
    context "itemのrakuten_urlに値が保存されている" do
      before do
        sign_in user
      ...
      it "選択された楽天リンク欄にDBに保存されているrakuten_urlと削除ボタンが表示されている", js: true do
        ...
        expect(page).to have_field('item_rakuten_url', with: 'http://image_url1', type: 'hidden')
      ...
      it "選択された楽天リンク欄の削除ボタンをクリックすると、”選択された楽天リンク”欄が”なし”という表記に変わりhidden_fieldからも削除され、更新ボタンをクリックするとrakuten_urlカラムの値も削除される", js: true do
        ...
        click_on '更新'
        item1.reload
        expect(item1.rakuten_url).to eq ''
      end
      ...
      it "新たな楽天URLを選択し、更新ボタンをクリックすると新たに選択したURLがrakuten_urlとして保存される", js: true do
        ...
        click_on '更新'
        item1.reload
        expect(item1.rakuten_url).to eq 'http://test2.com'
      end
    end

    context "itemのrakuten_urlに値が保存されていない" do
      before do
        sign_in user
      ...
      it "新たな楽天URLを選択し、更新ボタンをクリックすると新たに選択したURLがrakuten_urlとして保存される", js: true do
        ...
        click_on '更新'
        item2.reload
        expect(item2.rakuten_url).to eq 'http://test1.com'
      end
    end
  end
end
