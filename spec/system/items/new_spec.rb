...
RSpec.describe "items", type: :system do
  ...
  before do
    ...
    allow(RakutenWebService::Ichiba::Item).to receive(:search).and_return(
      [
        double("RakutenItem", name: "logicool G600", url: "http://test1.com", small_image_urls: ["http://image_url1"]),
        double("RakutenItem", name: "Razer NagaPro", url: "http://test2.com", small_image_urls: ["http://image_url2"]),
      ]
    )
  end
  ...
  describe "rakuten_url" do
    ...
    it "検索結果の各商品の”選択”ボタンをクリックすると、モーダルが閉じ、その商品のURLが”選択された楽天リンク”欄に表示され、hidden_fieldにも代入される", js: true do
      ...
      expect(page).to have_field('item_rakuten_url', with: 'http://test1.com', type: 'hidden')
    end
    ...
    it "選択された楽天リンク欄の削除ボタンをクリックすると、”選択された楽天リンク”欄が”なし”という表記に変わり、hidden_fieldからも削除される", js: true do
      ...
      expect(page).to have_field('item_rakuten_url', with: "", type: 'hidden')
    end

    it "選択された楽天リンク欄に選択したURLが表示された状態で登録ボタンをクリックするとitemsモデルのrakuten_urlカラムにデータが保存される（その他必須項目は入力されている状態）", js: true do
      find('#rakuten-link-button').click
      fill_in 'q', with: 'G600'
      click_on '検索'
      sleep 1
      find('.rakuten-add-button', match: :first).click
      fill_in 'item_name', with: 'G600'
      fill_in 'item_category_list', with: 'マウス'
      fill_in 'item_detail', with: '多ボタンマウス'
      click_button '登録'
      expect(Item.last.rakuten_url).to eq 'http://test1.com'
    end
    ...
    it "楽天リンクを選択した状態で、再度楽天のリンクを追加するボタンをクリックし、異なる商品を選択した場合その選択した商品のURLに選択された楽天リンク欄もhidden_fieldも入れ替わる", js: true do
      ...
      expect(page).to have_field('item_rakuten_url', with: 'http://test2.com', type: 'hidden')
    end
